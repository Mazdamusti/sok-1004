...
når vi plotter ett histogram brukes binwidt til å anngi hvor stor bredde(widt) eller spenn som skal til for at 2 observasjoner skal telles som 1. om jeg skulle forklart noen hvordan de skal bruke den ville jeg sagt ''forsiktig'' bra bruk av binwidt kan gi ett ''cleanere'' og mer oversiktlig plot, men setter du en for høy binwidt kan du miste verdier og miste oversikt.

i ett datasett med får observasjoner er det lønnsomt med en høy binwith for å få mer sammenheng. det motsattte.
...
#laster ned datasett og gir litt bedre navn til kolonnene slik at de blir lettere  å jobbe med vidre.
```
...
#mutater år til en integer og deler verdi slik at vi får tallene i milliarder
```

```{r}
df<-df %>% 
  mutate(gruppe=str_replace(gruppe,"sysselsatte","")) %>% 
  mutate(var=str_replace(var,"Kostnader til innleid personale","innleide")) %>% 
...
#lager noen egne lister vi skal bruke senere
```
...
df %>%
  ...
  filter(var %in% c(var_navn[1])) %>% 
  ggplot(aes(verdi))+
  geom_histogram(binwidth = 0.5, fill = "red", alpha = 0.5) +
  facet_grid(~ factor(gruppe, levels = c("20-49", "50-99", "100-199", "200-499", ">500")))+
  facet_grid(gruppe ~ næring)+
  labs(x = "Miliarder i kroner", y = "Antall")

# vi filrer ut variablene vi vil bruke til plottet og bruker ett histogram med binwidt 0.5
#vi bruker facet grid for å kunne lage ett større plot med flere figurer fra variablene vi velger ut.
```
...
# vi kjører samme kode med en ny variabel i filter nr2 
