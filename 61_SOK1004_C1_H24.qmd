...
# her lager vi datasettet om til e leselig form og bruker mutate til å endre variabelklassen for ''tid* til en integer fra en character slik det orginalt var slik at vi kan jobbe med det som en numerisk verdi videre. 
```
...
df %>%
  filter(tid %in% c(2000:2023)) %>%
  ggplot(aes(x = tid, y = verdi/1000, color = var)) +
  geom_line(size = 1.) +
  geom_point(size = 1) +
  theme (legend.position = "bottom") +
  scale_color_manual(values = c("BNP" = "blue", "MEMOBNP" = "red")) +
  scale_linetype_manual(values = c("BNP_løpende" = "solid", "BNP_faste" = "dashed")) +
  labs(
    title = "BNP per Person i Tusener av Kroner (2000-2023)",
    x = "År",
    y = "BNP per Person (1000 kr)",
    color = "Type"
  )
...
Når BNP skal analyseres ser vi på løpende og fast BNP.

Løpende bnp kan defineres som "grov" bnp der det ikke er tatt høyde for inflasjon. Når vi ikke tar høyde for dette kan det gi intrykk av stor økonomisk vekst uten at dette faktisk er tilfellet slik det vises på den blå linjen. Figuren kan gi intrykk av en tilsynelatende stor vekst fra 2020 jeg mistenker dette skyldes corona epidemien der statlige støtte til bedriftene som slet i perioden og økning i mengden penger i omsetning i landet sendte den løpende bnpen tilværs, før den daler ned igjen.

ser vi derimot på den faste BNP'en ( priser låst i 2015) der det er tatt høyde for dette ser vi at den reele økonomiske veksten har vert mye mindre, men fortsatt økende. ca 100 000 fra 2000-2023.

fast bnp og løpende bnp sammenlignes for å se den den faktiske og den ''uekte'' verdiøkningen.
...
#denne koden er veldig kul og den hjelper oss med flere ting. den utvider tabellen vi får fra df settet og sidestiller verdiene slik at vi kan regne ut på sammelinje, prosenter sammenligninger eller hva enn annet vi kunne ha lyst på. Navnene hentes fra ''var''(variabel) og verdiene hentes fra verdi.
...
# df_long <-df_wide %>%
df_long <- df_wide %>%
  pivot_longer(cols = c(DBNP, DMEMOBNP),
               names_to = "var",
               values_to = "verdi"
               )
```
...
df_long %>%
  ...
  + theme_minimal() %>%
  + theme (legend.position = "bottom") %>%
  + scale_color_discrete(name = "", labels = c("BNP, løpende priser", "BNP, faste priser"))
```

Som vi kan se på figuren er det negativ vekst tilstedet på ett par punkter. på punkter der veksten er under 0 er veksten negativ. første observasjoner er i 1982 og 1987. Dette vil si at det var fra er reel negativ vekst fra året før. mange variabler som kan være årsaken til dette, aksjemarkedet, corona. boligmarkeder etc. om vi ønsker å analysere økonomiske endringer i ettertid vil denne grafen være ett svært nyttig verktøy.
